...
export class PostgreSQLDatabase implements DatabaseInterface {
  ...
  async guardarInvestigador(datos: any) {
    try {
      ...
      const nombre = datos.nombreCompleto?.trim() || datos.nombre_completo?.trim() || ""
      ...
      if (!curp || curp === "") {
        const existenteNombre = await this.client.query(
          'SELECT * FROM investigadores WHERE "nombreCompleto" = $1',
          [nombre]
        )
  ...
  async verificarCredenciales(email: string, password: string) {
    try {
      ...
      return {
        ...
        user: {
          id: usuario.id,
          nombre: usuario.nombreCompleto || usuario.nombre_completo,
