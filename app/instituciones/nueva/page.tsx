...
interface FormData {
  ...
  imagenInstitucionUrl?: string
  imagenInstitucionPublicId?: string
...
export default function NuevaInstitucionPage() {
  ...
  const [imagenUploadStatus, setImagenUploadStatus] = useState<'idle' | 'uploading' | 'success' | 'error'>('idle')
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    imagenInstitucionUrl: "",
    imagenInstitucionPublicId: "",
  ...
  const imagenMostrada = imagenPreview || (formData.imagenInstitucionUrl ? formData.imagenInstitucionUrl : null)
  ...
  const handleContactoChange = (field: keyof ContactoInstitucional, value: string) => {
    ...
    // Para teléfono, solo permitir números y limitar a 10 dígitos (sin formateo automático)
    if (field === "telefono") {
  ...
  const handleImagenChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      setImagenError("La imagen excede el tamaño máximo de 5MB")
      setImagenUploadStatus('error')
      setImagenPreview(null)
      return
    }
    ...
    setImagenUploadStatus('uploading')

    try {
      const formDataUpload = new FormData()
      formDataUpload.append('file', file)

      const response = await fetch('/api/instituciones/upload-imagen', {
        method: 'POST',
        body: formDataUpload
      })
      ...
      setFormData(prev => ({
        ...prev,
        imagenInstitucionUrl: result.url || "",
        imagenInstitucionPublicId: result.publicId || ""
      }))

      setImagenPreview(result.url || null)
      setImagenUploadStatus('success')
      setImagenError(null)
      toast.success('Imagen subida correctamente')
    } catch (error) {
      console.error('Error al subir imagen de institución:', error)
      setImagenUploadStatus('error')
      const message = error instanceof Error ? error.message : 'Error desconocido al subir la imagen'
      setImagenError(message)
      setImagenPreview(null)
      setFormData(prev => ({
        ...prev,
        imagenInstitucionUrl: "",
  ...
  const validatePhone = (phone: string): boolean => {
    // Limpiar el teléfono de cualquier formato y verificar que tenga 10 dígitos
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.contactoInstitucional.email || !formData.contactoInstitucional.email.trim()) {
      newErrors.push({ field: "contactoInstitucional.email", message: "El email de contacto es obligatorio" })
    } else if (!validateEmail(formData.contactoInstitucional.email.trim())) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (imagenUploadStatus === 'uploading') {
    ...
    try {
      ...
      const institucionData = {
        ...
        imagenUrl: formData.imagenInstitucionUrl ? formData.imagenInstitucionUrl : null,
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      {imagenMostrada && (
                        <Button
                          ...
                          disabled={imagenUploadStatus === 'uploading'}
                        >
                          Quitar imagen
                        </Button>
                      )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <Label htmlFor="añoFundacion" className="text-blue-900">
                    ...
                    onChange={(e) => {
                      ...
                      if (value === '' || (!isNaN(Number(value)) && Number(value) >= 1500 && Number(value) <= new Date().getFullYear())) {
                        handleInputChange("añoFundacion", value)
                      }
                    }}
                    onBlur={(e) => {
                      ...
                      const value = e.target.value
                      if (value && (isNaN(Number(value)) || Number(value) < 1500 || Number(value) > new Date().getFullYear())) {
                        setErrors(prev => [...prev.filter(e => e.field !== "añoFundacion"), {
                          field: "añoFundacion",
                          message: `El año debe estar entre 1500 y ${new Date().getFullYear()}`
                        }])
                      }
                    }}
