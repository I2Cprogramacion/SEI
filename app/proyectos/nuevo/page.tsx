...
export default function NuevoProyectoPage() {
  ...
  const presupuestoOpciones = useMemo(() => {
    return Array.from({ length: 100 }, (_, idx) => {
      const amount = (idx + 1) * 10000
      return {
        value: amount.toString(),
        label: `${formatCurrency(amount)} MXN`
      }
    })
  }, [])
  ...
  const handlePresupuestoSelect = (value: string) => {
    setPresupuestoSeleccionado(value)
    if (value === "otro") {
      setFormData(prev => ({
        ...prev,
        presupuesto: "",
      ...
      const formatted = isNaN(amount) ? "" : `${formatCurrency(amount)}`
      setFormData(prev => ({
        ...prev,
        presupuesto: formatted ? `${formatted} MXN` : "",
        presupuestoOtro: ""
      }))
      setErrors(prev => prev.filter(error => error.field !== "presupuesto"))
    }
  }

  const handlePresupuestoOtroChange = (value: string) => {
    ...
    const formatted = `${formatCurrency(isNaN(amount) ? 0 : amount)}`
    setFormData(prev => ({
      ...prev,
      presupuestoOtro: formatted,
      presupuesto: `${formatted} MXN`
    }))
    setErrors(prev => prev.filter(error => error.field !== "presupuesto"))
  }
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select value={presupuestoSeleccionado} onValueChange={handlePresupuestoSelect}>
                      <SelectTrigger className={errors.some(e => e.field === "presupuesto") ? "border-red-300" : ""}>
                        <SelectValue placeholder="Selecciona un monto aproximado" />
                      </SelectTrigger>
                    ...
                    {presupuestoSeleccionado === "otro" && (
                      <Input
                        id="presupuesto-otro"
                        placeholder="$0"
                        value={formData.presupuestoOtro || ""}
                        onChange={(e) => handlePresupuestoOtroChange(e.target.value)}
                        className={errors.some(e => e.field === "presupuesto") ? "border-red-300 mt-2" : "mt-2"}
                      />
                    )}

                    <p className="text-xs text-blue-500">
                      Selecciona un rango o elige “Otro” para capturar un monto personalizado.
                    </p>
